...
//	glEnable(GL_CULL_FACE);
    ...
    g_Camera = std::make_shared<Camera>( glm::perspective( 45.0f, GetWidth() / (float)GetHeight(), 0.1f, 100.0f ) );
...
void MyApp::OnMouseEvent( int button, int action, int mods )
{
...
bool hold = false;
void MyApp::OnKeyboardEvent( int key, int scancode, int action, int mods )
{
    m_PickedIndex = -1;
...
void MyApp::OnMouseWheelEvent( double xoffset, double yoffset )
{
...
void MyApp::PickObject()
{
    Renderer::BeginPickingScene( g_Camera );

    Renderer::EndPickingScene( m_PickedIndex );
    Log::Info( "Picking {}", m_PickedIndex );
}
